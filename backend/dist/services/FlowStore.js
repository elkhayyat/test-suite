...
class FlowStore {
    ...
    async initialize() {
        try {
            ...
            SampleFlows_1.sampleFlows.forEach(flow => {
                this.flows.set(flow.id, flow);
            });
        }
    }
    async loadFlowsFromDatabase() {
        try {
    ...
    async createFlow(data) {
        ...
        try {
            const db = await (0, database_1.getDatabase)();
            await db.run(`INSERT INTO flows (id, project_id, folder_id, name, description, nodes, edges, created_at, updated_at) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, flow.id, flow.projectId, flow.folderId || null, flow.name, flow.description, JSON.stringify(flow.steps), JSON.stringify(flow.connections), flow.createdAt.toISOString(), flow.updatedAt.toISOString());
        }
        catch (error) {
    ...
    async updateFlow(id, data) {
        ...
        try {
            const db = await (0, database_1.getDatabase)();
            await db.run(`UPDATE flows 
         SET project_id = ?, folder_id = ?, name = ?, description = ?, nodes = ?, edges = ?, updated_at = ?
         WHERE id = ?`, updatedFlow.projectId, updatedFlow.folderId || null, updatedFlow.name, updatedFlow.description, JSON.stringify(updatedFlow.steps), JSON.stringify(updatedFlow.connections), updatedFlow.updatedAt.toISOString(), id);
        }
        catch (error) {
